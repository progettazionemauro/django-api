...
for markdown_file in "$markdown_dir"/*.md; do
  if [ -f "$markdown_file" ]; then
    ...
    title="${filename%.*}"  # Extract title from filename without extension
    new_filename="$title.md"  # New filename with title followed by .md
    ...
    echo "Title: $title"
    echo "File Name: $new_filename"  # Print the new filename
    echo "Content: $content"
    echo "Image Link: $image_link"
    echo "-------------------------"

    # Save the data to the Django database using the Django shell
    echo "from blog.models import Post; Post.objects.create(title=\"$title\", text=\"$content\", file_name=\"$new_filename\", image_name='', image_link=\"$image_link\")" | python3 manage.py shell

    # Log the details to the output file
    echo "Title: $title" >> "$output_file"
    echo "File Name: $new_filename" >> "$output_file"  # Log the new filename
