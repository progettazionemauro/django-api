...
# Check the operation argument (create or delete)
if [ "$1" == "create" ]; then
    ...
    # Delete the specified post file
    file_to_delete="$posts_dir/$2"
    if [ -f "$file_to_delete" ]; then
        rm "$file_to_delete"
        echo "Post deleted successfully: $file_to_delete"
    else
    ...
    echo "Error: Invalid operation. Use 'create' or 'delete'."
    exit 1
fi
